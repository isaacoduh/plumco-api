...
const createOrder = catchAsyncErrors(async (req, res) => {
  try {
    ...
    console.log(cartItems[0].price.toFixed(2));
      ...
      transactions: [
        {
          ...
          amount: { currency: "GBP", total: totalAmount.toFixed(2) },
    ...
    paypal.payment.create(create_payment_json, async (error, paymentInfo) => {
      if (error) {
        ...
        res
          .status(201)
          .json({ success: true, approvalURL, orderId: newOrder._id });
      }
    });
  } catch (error) {
    console.log(error);
...
const capturePayment = catchAsyncErrors(async (req, res) => {
  try {
...
const getAllOrdersByUser = catchAsyncErrors(async (req, res) => {
  try {
